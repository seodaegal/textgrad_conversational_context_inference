...
def load_task(task_name: str, evaluation_api: EngineLM, *args, **kwargs) -> Tuple[Dataset, Dataset, Callable]:
    ...
    elif task_name == "BBUNG_BBUNG":
        from textgrad.tasks.bbung_task import BBUNG
        from textgrad.autograd.string_based_ops import StringBasedFunction
        from .big_bench_hard import string_based_equality_fn
        evaluation_instruction = ""
        train_set = BBUNG("BBUNG_BBUNG", split="train", *args, **kwargs)
        val_set = BBUNG("BBUNG_BBUNG", split="val", *args, **kwargs)
        test_set = BBUNG("BBUNG_BBUNG", split="test", *args, **kwargs)
        eval_fn = StringBasedFunction(string_based_equality_fn, function_purpose="The runtime of string-based function that checks if the prediction is correct.")
        return train_set, val_set, test_set, eval_fn
    else:
